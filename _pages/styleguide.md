...
`a.yaml` through `z.yaml`, one file per initial letter.
**Only `_data/books-favorites.yaml` carries comments** (2026-08-07: "leave the comment in just one of them… to read
...
being in that file is the flag. It sat inside `_data/books/` as `_books.yaml` for a day; Jekyll reads underscore-prefixed data files without complaint, so it became a key in `site.data.books` that every loop had to skip by name. Moved back out on 2026-08-08 — `site.data.books` is letters only. The trade is that nothing now guarantees the two grids are
