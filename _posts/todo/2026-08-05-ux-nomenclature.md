...
This applies even to the Nomenclatures[^Nomenclature] that we use in email subjects, meeting titles, and even files & folder names.
...
I’m going to tell you a story from the early days of my career. A back-end developer and I used to build many software, and websites for clinics, and physicians. This was the time when websites were built on TABLES[^TABLE]. Assets in them were separate `.gif`, `.png`, or `.jpg` files.
...
- `2007-07-07-wedding-bride-with-parents.jpg`
- `2008-12-11-kid-first-day.jpg` etc.

Of course, if the file or folder are of a specific item/object but time if the variant, then it is better to have;

- `brajeshwar-profile-2010.jpg`
- `brajeshwar-profile-2020.jpg`
- `brajeshwar-profile-2026.jpg` etc.

Now, naming roads such as [Hollywood Boulevard](https://en.wikipedia.org/wiki/Hollywood_Boulevard), [MG Road](https://en.wikipedia.org/wiki/List_of_roads_named_after_Mahatma_Gandhi) makes sense as they are descriptive and tells you where they are. Unfortunately, this is not so for files, folders, groups, categories on your devices. Don’t name your files like `invoice-file.pdf`, or `Discussion Folder`, or named your team/group as `design-team` or `AI-group`.
