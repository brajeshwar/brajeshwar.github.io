...
The current set of [Eclipse EGL tools](https://www.eclipse.org/edt/) (delivered in the 0.7.0 release) are focused on simplifying the development of web solutions. With these tools one can:
...
There are 4 project templates given on the official [Eclipse EDT team blog](https://xeglblog.blogspot.in/2011/10/welcome-to-official-eclipse-edt-team.html) to kick-start new projects:
...
IBM's resolution to make the most out of its EGL tooling source, did not come as a bombshell to any person who has followed IBM's action over the last few years as it relates to the a high level Enterprise Generation Language, that outputs Enterprise Java, Java Script, HTML and other Web Oriented languages. In a nutshell, EGL is a higher level, universal application development language. This tool is constructed on the free EGL community Edition software that IBM plant in 2008. It gives IBM i shops another option for developing rich Web applications that run on the [IBM i operating system](https://www-03.ibm.com/systems/power/software/i/about.html) that can access RPG logic.
