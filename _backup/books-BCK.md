---
# NOT A PAGE. This file lives in `_backup/`, an underscore directory, which
# Jekyll never copies to the output — so it cannot publish itself at
# /books-BCK/ the way it would from _pages/ or the repo root (where
# jekyll-optional-front-matter would turn a bare .md into a page).
#
# Removed from _pages/books.md on 2026-08-01: "The prose from books will go.
# Remove from there and keep a backup books-BCK.md for me to decide what to do
# with it later."
#
# This is the COMPLETE prose block, verbatim, including the two footnote
# definitions that go with it. Nothing was reworded, reordered or trimmed.
#
# To restore: paste back below the grids in _pages/books.md. Note that the
# footnotes are what made that page get `container-ideal` from page.html (for
# sidenote positioning), so putting them back also restores that behaviour —
# and `.page > .album` at 100cqi is what keeps the grids at band width either
# way.
#
# A second copy sits outside the repo at
# ~/.claude/brajeshwar-content-backup-2026-08-01/books.md.original
---
