...
- **Footer**: three centred rows — **page links** (`_data/nav.yaml` `footer`, grouped, a subtle bullet between groups), **social icons** (`_data/nav.yaml` `social`; inline SVG from `_includes/icons/`), and a **copyright line** last. The **outer footer spans the full viewport** so the thin `border-top` is edge-to-edge; a `.footer-inner` wrapper keeps the content at the body width. *(Supersedes the earlier column-categorised grouping.)*
