...
| every off-site link | **no `target` at all** — the ↗ marks it, the reader decides where it opens |
...
⚠️ **AN OFF-SITE LINK FORCES NOTHING. NO `target`, NO `rel`, JUST THE ↗.** *"I don't want
to break convention and force a target=_blank."* Where a link opens belongs to the reader,
...
⚠️ **`_pages/devices.html` keeps `target="_oinam"`, which is HIS and predates all of this.**
It was edited twice during the above and restored both times; it is now the only place on
the site that forces a new window. **Leave it alone unless he asks.**
