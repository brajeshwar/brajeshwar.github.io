...
- **Footer simplified (this session; browser-verified desktop + mobile):** three centred rows — (1) flat wrapping **page links** (`_data/nav.yaml` `footer`, now a flat list of 13 in Brajeshwar's order incl. **Photos → `/photos/`**, a page that doesn't exist yet — intentional); (2) **social icons** (`_data/nav.yaml` `social`: RSS, Twitter, Github, Mastodon `rel=me`, Oinam, Memos, Instagram — URLs confirmed with Brajeshwar: Instagram = `instagram.com/oinam`, Memos = `memos.oinam.com`); (3) **copyright** line last (`© 2001–<year> Brajeshwar Oinam · N posts`, year via JS). Kept the **thin `border-top`**. **Icons live in `_includes/icons/*.svg`** — brand glyphs from **Simple Icons (CC0)**, `oinam`/`memos` hand-authored filled; footer template pulls them via a **data-driven `{% include {{ var }} %}`** (works). Files: `footer.html`, `3.1-footer.css`, `_data/nav.yaml`, `_includes/icons/`. **Uncommitted.**
