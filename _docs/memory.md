...
**Round three** (two more queue items): the strip's last tile became a button, all
its links were pointed into one reused tab, and that tab's name was corrected to the
site's existing `_oinam`.
...
| the home page | an **Albums** strip — **12 photographs and a "View More Albums" tile**, 13 in all; the heading links off-site |
| the footer | **Albums** → albums.oinam.com, the one off-site entry in `nav.yaml` |
| every off-site link | opens into **`target="_oinam"`** — one named window the browser reuses, not `_blank` |
...
⚠️ **`_oinam` IS THE SITE'S ONE OFF-SITE WINDOW, AND THE STRING LIVES IN THREE
