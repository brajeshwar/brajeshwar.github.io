...
| every off-site link | **`target="_blank" rel="noopener"`** — one rule, site-wide, 4,433 anchors |
...
⚠️ **NEW TABS ARE `_blank` + `rel="noopener"`, EVERYWHERE, AND THE NAMED-WINDOW IDEA IS
CLOSED.** It was tried and unwound the same afternoon; this is the whole episode, so nobody
reopens it:

1. Album links were given a shared window name so they would reuse ONE tab instead of
   opening a dozen — first `_oinam-albums`, then `_oinam` to match `/devices/`, which had
   used that name for years.
2. `rel="noopener"` was added to all of them on the reasoning that browsers imply it for
   `_blank` but not for a name. **That silently broke the reuse**, which is the opposite of
   what the reasoning predicted.
3. Measured in Chrome — two cross-origin links sharing one target name:
   ...
   "familiar" — which across origins ONLY the opener relationship provides. `noopener`
   severs exactly that. **So it is reuse OR noopener, never both**, and `rel="noreferrer"`
   is not a third option because it implies noopener.
4. Told the trade plainly, his answer was *"If it breaks norms, just use `_blank` for new
   tabs."* It does break norms — a cross-origin named target with no `noopener` is the
   reverse-tabnabbing shape linters flag — so everything is `_blank` now, `/devices/`
   included, and the `target` parameter and `nav.yaml` key that carried the name are gone.

⚠️ **So a click per photograph is a tab per photograph, and that is the accepted answer,
not an oversight.** `_blank` never had reuse to lose, which is why its `rel="noopener"`
costs nothing.
